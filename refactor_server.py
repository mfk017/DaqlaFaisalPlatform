...
def process_file(path):
    ...
    modified = content
    has_use_client = 'use client' in modified
    ...
    if modified != content:
        # Import injection
        if has_use_client:
            if 'useTranslation' not in modified:
            ...
            if 'getT' not in modified:
                modified = 'import { getT } from "@/lib/i18n";\n' + modified
            
            # API Routes (GET, POST, etc)
            for method in ['GET', 'POST', 'PUT', 'DELETE', 'PATCH']:
                method_sig = r'export (?:default )?async function ' + method + r'\s*\([^)]*\)\s*{'
                func_match = re.search(method_sig, modified)
                if func_match:
                    insert_pos = func_match.end()
                    # only add if not already added in this func
                    if 'await getT()' not in modified[insert_pos:insert_pos+30]:
                        modified = modified[:insert_pos] + '\n  const t = await getT();\n' + modified[insert_pos:]
            
            # Pages
            page_sig = r'export (?:default )?(?:async )?function [A-Za-z0-9_]+Page\s*\([^)]*\)\s*{'
            func_match = re.search(page_sig, modified)
            if func_match:
                insert_pos = func_match.end()
                if 'await getT()' not in modified[insert_pos:insert_pos+30]:
                    modified = modified[:insert_pos] + '\n  const t = await getT();\n' + modified[insert_pos:]
                    modified = modified.replace('export default function', 'export default async function')
                    modified = modified.replace('export function', 'export async function')
                    
        with open(path, 'w', encoding='utf-8') as f:
            f.write(modified)
        print(f"Refactored server/client file: {path}")
